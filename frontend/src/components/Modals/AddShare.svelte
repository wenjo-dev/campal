<script>
    import { request, timePickPresets, stores } from "../../helper";
    ...
    export const open = async (folderId) => {
        reset();
        let res = await request(`folder/info/${folderId}`);
        if(!res.ok) { return; }
    ...
    function updateFields() {
        ...
        if(!share.public.url.value || share.public.url.value == "") share.public.url.placeholder = encodeURIComponent(share.public.url.value);
    }

    async function confirm() {
        let res = await request("share", "post", {
            ...
            ...(share.public.protected && {password: share.public.protected.password}),
            ...(share.withUsers && { users: users.filter(e => e.selected).map(e => e.id) })
        })
        if(!res.ok) { return; }
        $notify = {action: "added", kind: "share"};
        ...
        {:else}
            ...
            {#if share.public.active}
                ...
                {#if share.public.protected.active}
                    ...
                    <input bind:value={share.public.protected.password} placeholder="password" type="text" autocomplete="off" class="input w-full input-bordered font-mono" />
                {/if}
            {/if}
            ...
            <div class="flex justify-between items-center mt-8">
                <button on:click={confirm} class="btn btn-neutral" disabled={(!share.withUsers && !share.public.active) || (share.public.active && (!share.public.url.check || (share.public.protected.active && (!share.public.protected.password || share.public.protected.password == "")))) || (share.withUsers && users.filter(e => e?.selected).length < 1)}>
                    <Icons icon="share"/>
                    share
                </button>
